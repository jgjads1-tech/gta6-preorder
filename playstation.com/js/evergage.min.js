...
(function configureEvergage() {
    try {
       Evergage.configure({
           account: "sonyinteractiveent",
           dataset: "sie_prod",
           cdnUrl: "https://cdn.evergage.com",
           trackerUrl: "https://sonyinteractiveent.us-5.evergage.com",
           siteConfigVersion: "67"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"USD","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableCrossDomainIframeCookies":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":false,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["accountHashedId","customerId","emailAddress","gpdc_anonId","profileId","psnId","sfmcContactKey"],"lastModified":1718810050806,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"secureCookie":false,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
SalesforceInteractions.init({
     ...
     * @param {string} path - Dot-notation path (e.g., 'user.subscriptions.loyalty.tier')
     * @param {*} defaultValue - Optional default value if path doesn't resolve
    ...
    /**
     * Safely check if a global object exists and has expected structure.
     * @param {string} globalName - Name of global variable (e.g., '_gpdc', 'digitalData')
     * @returns {Object|null} The global object or null if not available
     */
    const getGlobal = (globalName) => {
        try {
            const global = window[globalName];
            return global != null ? global : null;
        } catch (e) {
            return null;
        }
    };
    ...
    /**
     * Safely query DOM and return data attribute, with fallback.
     * @param {string} selector - CSS selector
     * @param {string} dataKey - Optional data key to retrieve (if not provided, returns element)
     * @returns {*} The data value, element, or null
     */
    const safeQueryData = (selector, dataKey = null) => {
        try {
    ...
    /**
     * Safely get product information from DOM data attributes.
     * Returns an object with defensive property access.
     * @returns {Object} Product information object with safe defaults
     */
    const getProductInformation = () => {
        try {
            const element = SalesforceInteractions.cashDom('.basepage.page.basicpage');
    ...
    /**
     * Creates an empty product info structure to prevent null reference errors.
     */
    const createEmptyProductInfo = () => ({
        productInfo: {
            conceptId: undefined,
            productId: undefined,
            skus: [],
        ...
        locale: undefined,
        'mercury-locale': undefined,
    });

    /**
     * Safely access productInfo properties with defaults.
     * @param {string} path - Property path within productInfo
     * @param {*} defaultValue - Default value if not found
     * @returns {*} The value or default
     */
    const getProductProp = (path, defaultValue = undefined) => {
        const productInfo = getProductInformation();
        return safeGet(productInfo, path, defaultValue);
    };

    /**
     * Check if we have valid product information for catalog tracking.
     * @returns {boolean} True if minimum required product data is available
     */
    const hasValidProductInfo = () => {
        const conceptId = getProductProp('productInfo.conceptId');
        return conceptId != null && conceptId !== '' && isValidEntry(conceptId);
    };

    /**
     * Safely access digitalData properties.
     * @param {string} path - Property path within digitalData
     * @param {*} defaultValue - Default value if not found
     * @returns {*} The value or default
     */
    const getDigitalDataProp = (path, defaultValue = undefined) => {
        const digitalData = getGlobal('digitalData');
        if (!digitalData) return defaultValue;
        return safeGet(digitalData, path, defaultValue);
    };

    /**
     * Safely access _gpdc properties.
     * @param {string} path - Property path within _gpdc
     * @param {*} defaultValue - Default value if not found
     * @returns {*} The value or default
     */
    const getGpdcProp = (path, defaultValue = undefined) => {
        const gpdc = getGlobal('_gpdc');
        if (!gpdc) return defaultValue;
        return safeGet(gpdc, path, defaultValue);
    };

    /**
     * Check if user is logged in via _gpdc.
     * @returns {boolean} True if user is confirmed logged in
     */
    const isUserLoggedIn = () => {
        return getGpdcProp('user.isLoggedIn') === true;
    };
    ...
    function isValidEntry(entry) {
        ...
        const invalidPatterns = [
            /\$\{.*?\}/, // Detects template strings or injection patterns
            /waitfor\/\*\*\/delay/, // Detects SQL injection patterns
            /select\+1/, // Detects SQL keywords
            /\bexpr\b/, // Detects usage of 'expr' command
            /and\s*['"]/i, // Detects 'and' followed by a quote (SQL injection)
            /convert\(.+?,.+?sys\.fn_sqlvarbasetostr/i, // Detects convert function with sys.fn_sqlvarbasetostr
            /HashBytes\(.+?'MD5'/i, // Detects HashBytes function with MD5
            /['"]\sand\s['"]/i, // Detects and surrounded by quotes
        ];
        return !invalidPatterns.some((pattern) => pattern.test(String(entry)));
    }
    ...
    const determineLocale = () => {
        try {
            const htmlElement = SalesforceInteractions.cashDom('html')[0];
            if (!htmlElement || !htmlElement.lang) return 'en_US'; // Safe default

            let locale = htmlElement.lang.replace(/-/gi, '_');
    ...
    const getPrice = () => {
        ...
        const normalizePriceData = (priceData) => {
            if (!priceData || typeof priceData !== 'object') return emptyPriceData;
            return {
                originalPriceFormatted: priceData.originalPriceFormatted || priceData.basePriceFormatted || priceData.priceFormatted || '',
                originalPriceValue: priceData.originalPriceValue || priceData.basePriceValue || priceData.priceValue || '',
                discountedPriceFormatted: priceData.discountedPriceFormatted || priceData.discountPriceFormatted || '',
                discountedPriceValue: priceData.discountedPriceValue || priceData.discountPriceValue || '',
                currency: priceData.currency || priceData.priceCurrency || '',
            };
        };

        // Attempt to access the data from telemetry meta
        try {
            const ctaButton = SalesforceInteractions.cashDom('button[data-qa="mfeCtaMain#cta#action"]');
            if (ctaButton && ctaButton.length > 0 && ctaButton[0].dataset) {
                const telemetryMeta = ctaButton[0].dataset.telemetryMeta;
                if (telemetryMeta) {
                    const parsed = JSON.parse(telemetryMeta);
                    const productPriceDetail = safeGet(parsed, 'productDetail.0.productPriceDetail.0');
                    if (productPriceDetail) {
                        return normalizePriceData(productPriceDetail);
                    }
                }
            }
        } catch (e) {
        ...
        try {
            ...
            const envScripts = Array.from(scripts).filter((script) => script.id && script.id.startsWith('env:'));

            const sku = getDigitalDataProp('product.0.productInfo.sku');
            if (!sku) return emptyPriceData;

            const parseAndCheckConceptId = (scriptContent) => {
                try {
                    const jsonData = JSON.parse(scriptContent);
                    if (jsonData.cache && 
                        (jsonData.cache[`GameCTA:ADD_TO_CART:ADD_TO_CART:${sku}:OUTRIGHT`] || 
                         jsonData.cache[`GameCTA:PREORDER:BUY_NOW:${sku}:OUTRIGHT`])) {
                        return jsonData;
                    }
                } catch (e) {
                    // Silent fail
            ...
            const matchingScripts = envScripts
                .map((script) => parseAndCheckConceptId(script.textContent))
                .filter(Boolean);

            for (let i = 0; i < matchingScripts.length; i++) {
                const addToCartCache = safeGet(matchingScripts[i], `cache.GameCTA:ADD_TO_CART:ADD_TO_CART:${sku}:OUTRIGHT`);
                const preOrderCache = safeGet(matchingScripts[i], `cache.GameCTA:PREORDER:BUY_NOW:${sku}:OUTRIGHT`);

                if (addToCartCache && addToCartCache.price) {
                    return normalizePriceData(addToCartCache.price);
                } else if (preOrderCache && preOrderCache.price) {
                    return normalizePriceData(preOrderCache.price);
                }
            }
        } catch (e) {
            // Silent fail
    ...
    const getProductFeatures = () => {
        ...
        try {
            const mfeCompatibilityNotices = SalesforceInteractions.cashDom('.psw-l-line-left.psw-l-space-x-3');
            if (!mfeCompatibilityNotices || mfeCompatibilityNotices.length === 0) {
                return emptyFeatures;
            }

            const generalFeatures = [];
            const onlineFeature = [];
            const numberOfLocalPlayers = [];

            for (let i = 0; i < mfeCompatibilityNotices.length; i++) {
                const noticeElement = mfeCompatibilityNotices.eq(i);
                if (!noticeElement) continue;

                const notice = noticeElement.text().toUpperCase();
                if (!notice) continue;

                if (notice.includes('ONLINE PLAY') && !notice.includes('ONLINE PLAYERS')) {
                    onlineFeature.push(notice);
                } else if (notice.includes('PLAYSTATION VR2') || notice.includes('PS VR2')) {
                    if (!generalFeatures.includes('PS VR2')) {
                        generalFeatures.push('PS VR2');
                    }
                } else if (notice.includes('PS VR')) {
                    if (!generalFeatures.includes('PS VR')) {
                        generalFeatures.push('PS VR');
                    }
                } else if ((notice.includes('PLAYERS') || notice.includes('PLAYER')) && 
                           !notice.includes('ONLINE PLAYERS') && 
                           !notice.includes('NETWORK PLAYERS')) {
                    numberOfLocalPlayers.push(notice);
                }
            }

            return { onlineFeature, generalFeatures, numberOfLocalPlayers };
        } catch (e) {
            return emptyFeatures;
        }
    };
    ...
    const buildPlatformsFlag = () => {
        try {
            const platformFlags = [];
            const platformIcons = SalesforceInteractions.cashDom('.txt-style-tag');
            if (platformIcons && platformIcons.length > 0) {
                for (let i = 0; i < platformIcons.length; i++) {
                    if (platformIcons[i] && platformIcons[i].innerText) {
    ...
    // ============================================================
    // CATALOG ATTRIBUTE BUILDERS (Defensive)
    // ============================================================

    /**
     * Builds catalog object attributes with defensive access patterns.
     * Each attribute function is wrapped to silently fail and return safe defaults.
     */
    const buildCatalogAttributes = () => ({
        name: () => getProductProp('gameInfo.name', ''),
        price: () => getProductProp('productInfo.price', 0),
        listPrice: () => {
            const priceData = getPrice();
            const formatted = priceData.originalPriceFormatted;
            if (!formatted) return 0;
            const parsed = parseFloat(String(formatted).replace(/[^0-9.-]/g, ''));
            return isNaN(parsed) ? 0 : parsed;
        },
        url: window.location.origin + window.location.pathname,
        imageUrl: () => {
            const locale = getProductProp('locale');
            if (locale === 'en-us') {
                return getProductProp('productInfo.image', '');
            }
            return '';
        },
        currency: () => getProductProp('productInfo.priceCurrency', ''),
        inventoryCount: 1,
        description: () => {
            try {
                let textBlock = safeQueryText('.textblock.parbase > .text-block');
                textBlock = textBlock.replace(/\n/g, '').substring(0, 250);

                if (textBlock.length === 0 || textBlock.includes('©')) {
                    const gameOverview = safeQueryText('[data-mfe-name="gameOverview"]')
                        .replace(/\n/g, '')
                        .substring(0, 250);
                    return gameOverview || '';
                }
        ...
        releaseDate: () => {
            const directDate = getProductProp('productInfo.releaseDate');
            if (directDate) return directDate;

            const skus = getProductProp('productInfo.skus', []);
            if (skus.length > 0 && skus[0]) {
                return skus[0].releaseDate || '';
            }
            return '';
        },
        publisherName: () => getProductProp('productInfo.publisher', ''),
    });

    /**
     * Builds related catalog objects with defensive access patterns.
     */
    const buildRelatedCatalogObjects = () => ({
        Category: () => {
            try {
                let platform = getProductProp('productInfo.platforms');
                let genres = getProductProp('productInfo.genres');

                // Fallback: try to get genres from skus
                if (!genres || genres.length === 0) {
                    genres = safeGet(getProductInformation(), 'productInfo.skus.0.genres', []);
                }

                // Fallback: try to build platforms from DOM
                if (!platform) {
                ...
                if (platform && platform.length > 0 && genres && genres.length > 0) {
        ...
        ProductType: () => {
            try {
                const productType = getProductProp('productInfo.productType');
                if (productType) return [productType];

                const skuProductType = safeGet(getProductInformation(), 'productInfo.skus.0.productType');
                if (skuProductType) return [skuProductType];
        ...
        Platform: () => {
            try {
                const platform = getProductProp('productInfo.platforms');
        ...
        Rating: () => {
            try {
                const locale = getProductProp('mercury-locale');
                const currency = getProductProp('productInfo.priceCurrency');

                // Prevent duplicate dimensions with multiple languages
                if (currency !== 'USD' && locale !== 'en_US') {
                    return [];
                }

                const contentRating = getProductProp('productInfo.contentRating');
                if (contentRating) return [contentRating];

                // Fallback to DOM
                const ratingElement = SalesforceInteractions.cashDom('.psw-l-line-left-top > a');
                if (ratingElement && ratingElement.length > 0) {
                    const trackData = ratingElement.data('track');
                    const label = safeGet(trackData, 'attributes.label');
        ...
        Discount: () => {
            try {
                const priceData = getPrice();
                const originalPriceValue = priceData.originalPriceValue;
                const discountedPriceValue = priceData.discountedPriceValue;

                if (originalPriceValue && discountedPriceValue) {
                    return Number(originalPriceValue) > Number(discountedPriceValue) 
                        ? ['DISCOUNTED_PRICE'] 
                        : ['FULL_PRICE'];
                }
                return ['FULL_PRICE'];
    ...
    const ATCRelated = {
        doATCrequest: function (game) {
            try {
                if (!game || !safeGet(game, 'gameData.standardData')) return;

                const ATCdata = this.getDataForATC(game);
                if (!ATCdata) return;

                window.dispatchEvent(
                    new CustomEvent('psw:message', {
                        detail: {
                            data: ATCdata,
                            type: 'ADD_TO_CART',
                        },
        ...
        getDataForATC: function (game) {
            try {
                const gameStandardData = safeGet(game, 'gameData.standardData');
                if (!gameStandardData || !gameStandardData.action || !gameStandardData.action.param) {
                    return null;
                }

                const extractedParams = {};
                gameStandardData.action.param.forEach((param) => {
                    if (param && param.name && param.value !== undefined) {
                        extractedParams[param.name] = param.value;
                    }
                });

                if (!extractedParams.conceptId || !extractedParams.skuId) {
                    return null;
                }

                const skuId = extractedParams.skuId;
                const titleId = skuId.includes('-') ? skuId.split('-')[1] : skuId;

                return {
                    ...extractedParams,
                    telemetryMeta: {
                        conceptId: extractedParams.conceptId,
                        productId: game.productId,
                        productDetail: [
                            {
                                productId: game.productId,
                                productName: game.name,
                                productToSkuIds: [skuId],
                            },
                        ],
                        skuDetail: [
                            {
    ...
    function addToCartObserver() {
        ...
        setInterval(() => {
            try {
                if (addToCartEventLogged) return;

                const commerceEvent = getDigitalDataProp('commerceEvent');
                if (!commerceEvent) return;

                const eventName = safeGet(commerceEvent, 'eventInfo.eventName', '');
                if (!eventName) return;

                if (eventName.includes('cart_add') || eventName.includes('add_pre_order')) {
                    addToCartEventLogged = true;

                    const id = getProductProp('productInfo.conceptId');
                    const price = getProductProp('productInfo.price', 0);
    ...
    // ============================================================
    // ISMATCH HELPER FUNCTIONS
    // ============================================================

    /**
     * Safely check URL pattern match.
     * @param {RegExp} pattern - URL pattern to test
     * @returns {boolean} True if URL matches pattern
     */
    const urlMatches = (pattern) => {
        try {
    ...
    /**
     * Safely check if digitalData pageName contains a string.
     * @param {string} value - Value to check for
     * @returns {boolean} True if pageName contains value
     */
    const pageNameContains = (value) => {
        const pageName = getDigitalDataProp('page.pageInfo.pageName', '');
        return pageName.includes(value);
    };

    /**
     * Safely check if digitalData pageName equals a string.
     * @param {string} value - Value to check for
     * @returns {boolean} True if pageName equals value
     */
    const pageNameEquals = (value) => {
        return getDigitalDataProp('page.pageInfo.pageName') === value;
    };

    /**
     * Safely check if digitalData primaryCategory equals a string.
     * @param {string} value - Value to check for
     * @returns {boolean} True if primaryCategory equals value
     */
    const primaryCategoryEquals = (value) => {
        return getDigitalDataProp('page.pageInfo.category.primaryCategory') === value;
    };
    ...
    const sitemapConfig = {
        global: {
            ...
            onActionEvent: (actionEvent) => {
                try {
                    ...
                    const psnIdParam = Evergage.util.getParameterByName('psnId') || Evergage.util.getParameterByName('psnid');
                    
                    // Grab email parameter that is 'e' - email address should be encoded for privacy.
                    const email_enc = Evergage.util.getParameterByName('e');

                    // Grab MCP_BN from URL - Passed by Abandon Browse campaign when we want to trigger ATC event
                    const mcp_bn = SalesforceInteractions.mcis.getParameterByName('MCP_BN');
                    ...
                    if (psnIdParam && psnIdParam !== 'network_psn') {
                        actionEvent.user.attributes.psnId = psnIdParam;
                    }

                    // If a url parameter of 'e' is passed, which should be encoded email, pull value and decode
                    if (email_enc) {
                        try {
                            const email = atob(email_enc);
                            if (validateEmail(email)) {
                                actionEvent.user.attributes.emailAddress = email;
                                actionEvent.user.attributes.sfmcContactKey = email;
                            }
                        } catch (e) {
                            // Silent fail - invalid base64
                        }
                    }

                    // If MCP_BN = TRUE, trigger atcTrigger event
                    if (mcp_bn && mcp_bn !== '' && hasValidProductInfo()) {
                        const conceptId = getProductProp('productInfo.conceptId');
                        const skus = getProductProp('productInfo.skus', []);
                        const skuId = skus.length > 0 && skus[0] ? skus[0].skuId : null;
                        const productId = getProductProp('productInfo.productId');
                        const name = getProductProp('gameInfo.name');

                        if (conceptId && skuId) {
                            ATCRelated.doATCrequest({
                                gameData: {
                                    standardData: {
                                        action: {
                                            param: [
                                                { name: 'conceptId', value: conceptId },
                    ...
                    // If user is logged in, attempt to pull attributes from _gpdc object on DOM
                    if (isUserLoggedIn()) {
                        // If hashed account id value is populated (i.e. user is signed in) pull value
                        const encryptedId = getGpdcProp('user.encrypted_id');
                        if (encryptedId != null) {
                            actionEvent.user.attributes.accountHashedId = encryptedId;
                        }

                        // If handle is populated (i.e. user is signed in) pull value
                        const handle = getGpdcProp('user.handle');
                        if (handle != null) {
                            actionEvent.user.attributes.psnHandle = handle;
                        }

                        // If Plus tier is populated, pull value
                        const plusTier = getGpdcProp('user.subscriptions.plus.tier');
                        if (plusTier != null) {
                            actionEvent.user.attributes.plus_tier = plusTier;
                        }

                        const age = getGpdcProp('user.age');
                        if (age != null) {
                            actionEvent.user.attributes.age = age;
                        }

                        const formattedLocale = getGpdcProp('config.formattedLocale');
                        if (formattedLocale != null) {
                            actionEvent.user.attributes.locale = formattedLocale;
                        }
                    }
                    ...
                    // If anything fails in onActionEvent, return the actionEvent as-is to prevent blocking
                    console.log('MCP: ', actionEvent);
                    return actionEvent;
                }
            },
        ...
        pageTypes: [
            {
                name: 'Homepage',
                isMatch: () => primaryCategoryEquals('Home') || pageNameEquals('web:pdc:home'),
                interaction: {
                    name: 'Homepage',
                },
                listeners: [
                    SalesforceInteractions.listener('click', '[data-dtm-label="Discover PlayStation Plus"]', () => {
                        SalesforceInteractions.sendEvent({
                            interaction: {
                                name: 'Discover Playstation Plus',
                            },
                        });
                    }),
                ],
            },
            {
                ...
                isMatch: () => {
                    return urlMatches(/\/games/) && 
                           hasValidProductInfo() && 
                           pageNameContains('web:pdc:games:');
                },
                interaction: {
                    ...
                    catalogObject: {
                        ...
                        id: () => {
                            const conceptId = getProductProp('productInfo.conceptId');
                            return (conceptId && isValidEntry(conceptId)) ? conceptId : '';
                        },
                        attributes: buildCatalogAttributes(),
                        relatedCatalogObjects: buildRelatedCatalogObjects(),
                    },
                },
            },
            {
                name: 'PlayStation 5',
                isMatch: () => urlMatches(/\/ps5/) && window.location.pathname.split('/').length < 5,
                interaction: {
                    name: 'PlayStation 5',
                },
            },
            {
                name: 'Accessories',
                isMatch: () => urlMatches(/\/accessories/) && pageNameEquals('web:pdc:accessories'),
                interaction: {
                    name: 'Accessories',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/dualsense-wireless-controller/),
                interaction: {
                    name: 'DualSense Wireless Controller',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/dualsense-edge-wireless-controller/),
                interaction: {
                    name: 'DualSense Edge Wireless Controller',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/ps5-console-covers/),
                interaction: {
                    name: 'PlayStation 5 Console Covers',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/pulse-3d-wireless-headset/),
                interaction: {
                    name: 'Pulse 3D Wireless Headset',
                },
            },
            {
                name: 'Media Remote',
                isMatch: () => urlMatches(/\/accessories\/media-remote/),
                interaction: {
                    name: 'Media Remote',
                },
            },
            {
                name: 'HD Camera',
                isMatch: () => urlMatches(/\/accessories\/hd-camera/),
                interaction: {
                    name: 'HD Camera',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps5\/ps5-features/),
                interaction: {
                    name: 'PlayStation 5 Features',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps5\/ps5-entertainment/),
                interaction: {
                    name: 'PlayStation 5 Entertainment',
                },
            },
            ...
            {
                name: 'PlayStation 4',
                isMatch: () => urlMatches(/\/ps4/) && pageNameEquals('web:pdc:ps4'),
                interaction: {
                    name: 'PlayStation 4',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps4\/ps4-pro/),
                interaction: {
                    name: 'PlayStation 4 Pro',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps4\/tech-specs/),
                interaction: {
                    name: 'PlayStation 4 Tech Specs',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/dualshock-4-wireless-controller/),
                interaction: {
                    name: 'DualShock 4 Wireless Controller',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/gold-wireless-headset/),
                interaction: {
                    name: 'Gold Wireless Headset',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/additional-ps4-gaming-controllers/),
                interaction: {
                    name: 'Additional PS4 Controllers',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/platinum-wireless-headset/),
                interaction: {
                    name: 'Platinum Wireless Headset',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/accessories\/playstation-camera/),
                interaction: {
                    name: 'PlayStation Camera',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps4\/ps4-entertainment/),
                interaction: {
                    name: 'PlayStation 4 Entertainment',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps4\/ps4-games/),
                interaction: {
                    name: 'PlayStation 4 Games',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps-vr\/ps-vr-games/) && pageNameEquals('web:pdc:ps-vr:ps-vr-games'),
                interaction: {
                    name: 'PlayStation VR Games',
                },
            },
            {
                name: 'PlayStation Plus Whats New',
                isMatch: () => urlMatches(/\/ps-plus\/whats-new/) && pageNameEquals('web:pdc:ps-plus:whats-new'),
                interaction: {
                    name: 'PlayStation Plus Whats New',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps-plus\/games/) && pageNameEquals('web:pdc:ps-plus:games'),
                interaction: {
                    name: 'PlayStation Plus Games',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps-now\/ps-now-games/),
                interaction: {
                    name: 'PlayStation Now Games',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps-plus/) && pageNameEquals('web:pdc:ps-plus'),
                interaction: {
                    name: 'PlayStation Plus',
                },
                listeners: [
                    SalesforceInteractions.listener('click', "[data-dtm-label='Buy now'", () => {
                        SalesforceInteractions.sendEvent({
                            interaction: {
                                name: 'Buy Now - PlayStation Plus',
                            },
                        });
                    }),
                ],
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps-plus\/join-ps-plus/),
                interaction: {
                    name: 'Join PlayStation Plus',
                },
            },
            {
                name: 'PlayStation VR',
                isMatch: () => urlMatches(/\/ps-vr/) && pageNameEquals('web:pdc:ps-vr'),
                interaction: {
                    name: 'PlayStation VR',
                },
            },
            {
                name: 'PlayStation VR Tech Specs',
                isMatch: () => urlMatches(/\/ps-vr\/tech-specs/),
                interaction: {
                    name: 'PlayStation VR Tech Specs',
                },
            },
            {
                name: 'PlayStation VR Accessories',
                isMatch: () => urlMatches(/\/ps-vr\/accessories/),
                interaction: {
                    name: 'PlayStation VR Accessories',
                },
            },
            {
                name: 'PlayStation VR Bundles',
                isMatch: () => urlMatches(/\/ps-vr\/bundles/),
                interaction: {
                    name: 'PlayStation VR Bundles',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps-vr2/) && pageNameEquals('web:pdc:ps-vr2'),
                interaction: {
                    name: 'PlayStation VR2',
                },
            },
            {
                name: 'PlayStation VR2 Games',
                isMatch: () => urlMatches(/\/ps-vr2\/games/) && pageNameEquals('web:pdc:ps-vr2:games'),
                interaction: {
                    name: 'PlayStation VR2 Games',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/playstation-network/),
                interaction: {
                    name: 'PlayStation Network',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/ps-now/),
                interaction: {
                    name: 'PlayStation Now',
                },
                listeners: [
                    SalesforceInteractions.listener('click', "[data-dtm-label='See all the games'", () => {
                        SalesforceInteractions.sendEvent({
                            interaction: {
                                name: 'See all the games - PS Now',
                            },
                        });
                    }),
                ],
            },
            {
                ...
                isMatch: () => urlMatches(/\/about-playstation-store/),
                interaction: {
                    name: 'About PlayStation Store',
                },
            },
            ...
            },
            {
                ...
                isMatch: () => urlMatches(/\/corporate\/about-us/),
                interaction: {
                    name: 'About PlayStation',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/playstation-stars/),
                interaction: {
                    name: 'PlayStation Stars',
                },
            },
            {
                ...
                isMatch: () => urlMatches(/\/editorial\/this-month-on-playstation/),
                interaction: {
                    name: 'This Month On PlayStation',
                },
            },
    ...
    if (sitemapConfig && 
        (window.location.origin === 'https://www.playstation.com' || 
         window.location.origin === 'https://direct.playstation.com')) {
